...
public class SecurityTest {
    ...
    public void authenticate_user_is_null() {
        ...
        when(userMock1.getPassword()).thenReturn("test_password");
        when(userMock1.getEmail()).thenReturn("test_email");
    ...
    public void authenticate_password_found() {
        //User user = new User();
        when(userServiceMock.getUserByEmail(any())).thenReturn(userMock1);



        when(userMock2.getPassword()).thenReturn("test_password");
        when(userMock2.getEmail()).thenReturn("test_email");
        String expectedPass = Base64.getEncoder().encodeToString(userMock2.getPassword().concat(userMock2.getEmail()).getBytes(StandardCharsets.UTF_8));

        when(userMock2.getPassword()).thenReturn(expectedPass);
        when(userMock1.getPassword()).thenReturn(expectedPass);
    ...
    public void authenticate_password_not_found() {
        when(userServiceMock.getUserByEmail(any())).thenReturn(userMock2);
    ...
    public void validateSession1_is_not_null() {
        ...
        when(userServiceMock.getUserBySessionId(any())).thenReturn(new User());
        Object response = security.validateSession(httpServletRequestMock);
        assertNotNull(response);
    }
    ...
    public void validSession_null() {
        when(userServiceMock.getUserBySessionId(any())).thenReturn(null);
        boolean response = security.validSession(UUID.randomUUID());
